...
- React side: download UI (port of Electron frontend patterns), library UI, settings. Talks to NestJS API via `packages/api-client`. Design tokens in `index.css` (`ink-*`, `bg-app/sidebar/content/card/elevated/player`, `accent-*`, `line*`, `radius-*`, `shadow-elev-*`). All components use tokens — no raw `zinc-*`/`blue-*` colors.
- **UI components:** `Button` (accent/secondary/ghost/outline/danger + icon sizes), `Card`, `Input`, `Slider`, `Progress`, `Badge`, `Skeleton`, `Spinner`, `EmptyState`, `ErrorState`, `Dialog`, `ContextMenu` (positioned dropdown with submenus + keyboard nav).
- **Pages:** `HomePage` (greeting, recently played grid, favorites list, recently added, skeleton loading), `LibraryPage` (tab filters, songs/artists/albums/playlists grid+list, search/filter), `SearchPage` (debounced API search, categorized results, keyboard navigation), `DownloadsPage` (progress with stages, cancel, retry), `SavedPage` (YouTube playlists with iframe embeds), `SettingsPage` (API URL, bitrate, transcode, iframe preview), `AuthPage` (login/register).
- **SongContextMenu:** right-click or ⋮ button on any `SongRow` → Play, Play Next, Add to Queue, Add to Favorites, Add to Playlist (submenu with playlist list + new playlist), Delete (disabled placeholder).
...
- **Player panels:** `QueuePanel` (right-side slide-in, 320px, shows Now Playing / Next Up / Previously, move up/down + remove per row, clear all), `LyricsPanel` (right-side slide-in, 384px, fetches synced lyrics via API, auto-scroll to active line with smooth scroll, unsynced fallback, empty state), `ExpandedPlayer` (full-screen overlay, large artwork left, synced lyrics right, transport controls + favorite toggle, Escape to close). All toggled from `NowPlayingBar` buttons; artwork/track area clickable to open `ExpandedPlayer`. `PlayerContext` exposes `playNext`, `addToQueue`, `removeFromQueue`, `clearQueue`, `moveInQueue`.
